...
int main(int argc, char** argv) {
  if (argc != 4) {
    ...
    std::cout << "Arguments are: input_stream, output_file, updates_in_sketch" << std::endl;
    exit(EXIT_FAILURE);
  }

  std::string input  = argv[1];
  std::string output = argv[2];
  int updates_in_sketch = std::stoi(argv[3]); // number of bytes it takes to represent a sketch
