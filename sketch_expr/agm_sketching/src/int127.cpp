...
uint128_t int127::toBoostUInt128() const {
  if (m_value & high_bitmask) throw InvalidUInt128CastException();
  return m_value;
}

int127::int127(const int127 &other) { m_value = other.m_value; }

int127::int127(const int128_t &other) {
  if (abs(other) & high_bitmask) throw Int127OverflowException();
  if (other < 0) {
    m_value = static_cast<uint128_t>((-other) + high_bitmask);
  } else
    m_value = static_cast<uint128_t>(other);
}

int127::int127(const unsigned long &other) { m_value = other; }
...
int127 &operator+=(int127 &lhs, const int127 &rhs) {
  ...
  if (abs(temp) & high_bitmask) throw Int127OverflowException();
  if (temp < 0) {
    lhs.m_value = static_cast<uint128_t>((-temp) + high_bitmask);
  } else
    lhs.m_value = static_cast<uint128_t>(temp);
  return lhs;
}
