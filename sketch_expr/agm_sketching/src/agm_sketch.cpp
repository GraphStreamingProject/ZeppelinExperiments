...
AGM_Sketch::AGM_Sketch(vec_t n, long seed)
    : seed(seed),
      n(n),
      large_prime(
          PrimeGenerator::generate_prime(static_cast<ubucket_t>(n) * n)) {
...
void AGM_Sketch::update(Update update) {
  ...
  for (unsigned i = 0; i < num_buckets; ++i) {
    ...
    XXH64_hash_t bucket_seed =
          Bucket_Boruvka::gen_bucket_seed(i, seed);
...
AGM_Sketch operator+(const AGM_Sketch &sketch1, const AGM_Sketch &sketch2) {
  assert(sketch1.n == sketch2.n);
  assert(sketch1.seed == sketch2.seed);
  assert(sketch1.large_prime == sketch2.large_prime);
  AGM_Sketch result = AGM_Sketch(sketch1.n, sketch1.seed);
...
AGM_Sketch &operator+=(AGM_Sketch &sketch1, const AGM_Sketch &sketch2) {
  assert(sketch1.n == sketch2.n);
  assert(sketch1.seed == sketch2.seed);
  assert(sketch1.large_prime == sketch2.large_prime);
...
AGM_Sketch operator*(const AGM_Sketch &sketch1, long scaling_factor) {
  AGM_Sketch result = AGM_Sketch(sketch1.n, sketch1.seed);
...
std::ostream &operator<<(std::ostream &os, const AGM_Sketch &sketch) {
  ...
  for (unsigned i = 0; i < num_buckets; ++i) {
    for (unsigned j = 0; j < num_guesses; ++j) {
      ...
      XXH64_hash_t bucket_seed =
          Bucket_Boruvka::gen_bucket_seed(bucket_id, sketch.seed);
      ...
      os << std::endl
         ...
         << "c:" << bucket.c << std::endl
         << "r:" << r << std::endl
         << (bucket.is_good(sketch.n, sketch.large_prime, bucket_seed, r,
                            1 << j)
                 ? "good"
                 : "bad")
         << std::endl;
