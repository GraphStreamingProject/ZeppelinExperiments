...
int main(int argc, char** argv) {
  ...
  const auto opt_repeats = [&]() { parse.push_back(arg_repeats); };
  ...
  const auto opt_point = [&]() { point_queries = true; };
  ...
  const auto print_usage = [&]() {
    ...
    std::cout << "[--point], [--repeat <num_repeats>], [--burst <num_grouped> <ins_btwn_qry>]"
              << std::endl;
    ...
    std::cout << "--point: [OPTIONAL] if present then use point queries instead of spanning forest "
                 "queries"
              << std::endl;
    ...
    std::cout << "--burst <num_grouped> <ins_btwn_qry>: [OPTIONAL] if present then queries should "
                 "be bursty"
              << std::endl;
    std::cout << "  num_grouped:   specifies how many queries should be grouped together"
              << std::endl;
    std::cout << "  ins_btwn_qry:  specifies the number of insertions to perform between each query"
              << std::endl;
  };
  ...
  double unrounded_gf = (42 * pow(log2(num_nodes), 2) / (log2(3) - 1)) / 100;
  int gutter_factor = -1*((int) unrounded_gf);
  sys_config conf {std::thread::hardware_concurrency() - 1, 0, gutter_factor, false, true};
  auto graph_config = create_graph_config(conf);
  ...
  if (repeats > 1) std::cout << "REPEATING the stream " << repeats << " times." << std::endl;
  ...
  if (num_bursts > 0) {
    if (num_updates / num_bursts < (size_t)ins_btwn_qrys * (num_grouped - 1)) {
      std::cout << "Too many bursts or too many insertion between queries, "
                   "updates between bursts is not positive."
                << std::endl;
      exit(EXIT_FAILURE);
    }
    upd_per_query =
        num_updates * (repeats / 2 + 1) / num_bursts - ins_btwn_qrys * (num_grouped - 1);
    query_idx = upd_per_query;
    stream.register_query(query_idx);  // register first query
    if (bursts) {
      std::cout << "Total number of updates = " << num_updates * repeats << std::endl;
      std::cout << "Perfoming " << num_queries << " queries in bursts with " << ins_btwn_qrys
                << " updates between queries within a burst." << std::endl;
      std::cout << num_grouped << " queries in a burst, and " << upd_per_query 
                << " updates between bursts." << std::endl;
    } else {
      std::cout << "Total number of updates = " << num_updates * repeats << std::endl;
      std::cout << "perfoming " << num_queries << " queries: one every " 
                << upd_per_query << std::endl;
    }
  }
      ...
      if (upd.second == END_OF_FILE)
        ...
        if (thr_id > 0) {
          ...
          if (point_queries) {
            ...
            bool connected = g.point_query(a, b);
            ...
            auto cc_temp = g.flush_end;
            int x = 0;
            for (int i = 0; i < 99; i++) {
              a = rand_node(rand_engine);
              b = rand_node(rand_engine);
              connected = g.point_query(a, b);
              x += connected;
            }
            auto end = std::chrono::steady_clock::now();

            std::cout << x << std::endl;
            cc_status_out << "Query completed, " << a << " and " << b << " connected: " << connected
                          << std::endl;
            cc_status_out << "Total query latency = "
                          << std::chrono::duration<double>(end - cc_start).count() << std::endl;
            cc_status_out << "Flush latency       = " << flush.count() << std::endl;
            cc_status_out << "CC alg latency      = "
                          << std::chrono::duration<double>(end - cc_temp).count() << std::endl;

          } else {
            size_t num_CC = g.connected_components(true).size();
            auto end = std::chrono::steady_clock::now();
            std::cout << "QUERY DONE at index " << query_idx << " Found " << num_CC
                      << " connected components" << std::endl;
            cc_status_out << "Query completed, number of CCs: " << num_CC << std::endl;
            cc_status_out << "Total query latency = "
                          << std::chrono::duration<double>(end - cc_start).count() << std::endl;
            cc_status_out << "Flush latency       = "
                          << std::chrono::duration<double>(g.flush_end - g.flush_start).count()
                          << std::endl;
            cc_status_out << "CC alg latency      = "
                          << std::chrono::duration<double>(end - g.flush_end).count() << std::endl;
          }
          ...
          if (num_queries > 1) {
            ...
            if (query_idx < num_updates * (repeated / 2 + 1)) {  // then do register as normal
              if (!stream.register_query(
                      (query_idx % num_updates) == 0 ? 1 : query_idx % num_updates)) {
            ...
            num_queries--;
            std::cout << "Registered next query at " << query_idx << " -> "
                      << query_idx % num_updates << std::endl;
          }
          num_query_ready--;
          query_done = true;
          lk.unlock();
        ...
        throw std::invalid_argument("Did not recognize edge type!");
    }
  };
    ...
    if (repeats - r > 1) {
      ...
      if (repeated % 2 == 0) {
        ...
        if (!stream.register_query(stream_reset_query == 0 ? 1 : stream_reset_query.load())) {
          std::cout << "Failed to register query, when resetting stream, at index " << query_idx
                    << std::endl;
          exit(EXIT_FAILURE);
        }
      }
  ...
  num_CC = g.connected_components(true).size();
  ...
  cc_status_out << "Total query latency = " << std::chrono::duration<double>(end - cc_start).count()
                << std::endl;
  cc_status_out << "Flush latency       = "
                << std::chrono::duration<double>(g.flush_end - g.flush_start).count() << std::endl;
  cc_status_out << "CC alg latency      = "
                << std::chrono::duration<double>(end - g.flush_end).count() << std::endl;
}
